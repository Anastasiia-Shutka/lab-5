class RaceHorse:
    ...
    def __init__(self, name, speed, age):
        """
        Initializes a horse object with the given name and age.
        """
        self.name = name
        self.speed = speed
        self.age = age
...
class Race:
    """
    Class representing a horse race.
    """
    def __init__(self):
        """
        Initializes a race object with an empty list of participants.
        """
        self.participants = []

    def add_horse(self, horse):
        """
        Adds a horse to the race if it is between 3 and 7 years old (inclusive).
        """
        if 3 <= horse.age <= 7:
        else:
            print(f"Sorry, {horse.name} is not eligible for the race.")
        self.participants.append(horse)

    def remove_horse(self, horse):
        """
        Removes a horse from the race.
        """
    ...
    def race(self):
        """
        Conducts a race and prints the results.
        """
